...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(user.password, form.password.data):
            ...
            flash('Bad email/password.Try again!!', 'danger')
...
def reset_request():
    if current_user.is_authenticated:
        return redirect(url_for('main.home'))  # Because we want them to logged out before they reset their password
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash(f'Your password is updated!! You can login now', 'success')
        return redirect(url_for('users.login'))
